...
def make_static_linux(src_lib, dst_release, header_file_mappings, header_files_src_base='./'):
    if os.path.exists(dst_release):
        shutil.rmtree(dst_release)

    os.makedirs(dst_release)
    shutil.copy(src_lib, dst_release)

    release_path = dst_release + '/include'
    for (src, dst) in header_file_mappings.items():
        copy_file(os.path.join(header_files_src_base, src),
                  os.path.join(release_path, dst, src[src.rfind("/") + 1:]))
