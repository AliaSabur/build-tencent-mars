...
{
#if defined(_MSVC_STL_VERSION) && (_MSVC_STL_VERSION >= 143)   /* VS 2022+ */
    // v143 STL 已删除 seekpos()/get_fpos_t()，直接显式转换即可
    return static_cast<std::fpos_t>(static_cast<std::streamoff>(pos));
#elif defined(_CPPLIB_VER) || defined(__IBMCPP__)              /* 旧 Dinkumware */
    return pos.seekpos();
#else                                                          /* 其他实现 */
